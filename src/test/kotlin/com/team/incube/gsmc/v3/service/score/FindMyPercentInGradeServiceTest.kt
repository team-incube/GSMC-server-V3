...
class FindMyPercentInGradeServiceTest :
    BehaviorSpec({
        ...
        Given("학년에 여러 반의 학생들이 있고 현재 학생이 중간 순위인 경우") {
            ...
            every { c.calculateTotalScoreByMemberIdService.execute(1L, true) } returns GetTotalScoreResponse(60)
            every { c.calculateTotalScoreByMemberIdService.execute(2L, true) } returns GetTotalScoreResponse(75)
            every { c.calculateTotalScoreByMemberIdService.execute(3L, true) } returns GetTotalScoreResponse(80)
            every { c.calculateTotalScoreByMemberIdService.execute(4L, true) } returns GetTotalScoreResponse(90)
